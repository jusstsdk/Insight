...
export default function DrawerListItems({
  ...
  const listItem = (
    <>
      ...
      >
        <ListItemIcon
          key={`subtitle_${subtitle._id}_listItemIcon_${subtitle_index}`}
        >
          <BookIcon
            key={`subtitle_${subtitle._id}_bookIcon_${subtitle_index}`}
          />
        </ListItemIcon>
        <ListItemText
        ...
        {openCollapses[subtitle_index] ? (
          <ExpandLess
            key={`subtitle_${subtitle._id}_ExpandLess_${subtitle_index}`}
          />
        ) : (
          <ExpandMore
            key={`subtitle_${subtitle._id}_ExpandMore_${subtitle_index}`}
          />
        )}
      </ListItem>
    </>
          ...
          >
            ...
            >
              {singleContent.type === "Video" ? (
                <OndemandVideoIcon
                  className={
                    singleContent.isWatched ? "success ms-0 me-3" : "me-3"
                  }
                  key={`subtitle_${subtitle._id}_content_${singleContent._id}_OndemandVideoIcon_${singleContent_index}`}
                />
              ) : singleContent.type === "Content" ? (
                <QuizIcon
                  className={
                    singleContent.isWatched
                      ? "success me-3"
                      : singleContent.isWatched
                        ? "error me-3"
                        : "me-3"
                ...
                <QuizIcon
                  className={
                    ...
                    singleContent.receivedGrade / singleContent.maxGrade >= 0.5
                      ? "success me-3"
                      : singleContent.isSolved &&
                          singleContent.receivedGrade / singleContent.maxGrade <
                            0.5
                        ? "error me-3"
                        : "me-3"
                  }
