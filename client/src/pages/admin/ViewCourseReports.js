...
function ViewCourseReports() {
	...
	const ResolvingReport = (report) => {
		...
		let newReports = Reports.map((oldReport) =>
			oldReport._id === report._id ? { ...oldReport, isResolved: true } : oldReport
		);
	...
	useEffect(() => {
		...
		let resolved = location.state.reports.filter((report) => report.isResolved === true);
		let pending = location.state.reports.filter((report) => report.isResolved === false);
	...
	useEffect(() => {
		if (selectedItem === "Seen") {
			...
			let UnseenAndPending = pending.filter((report) => report.isSeen === false);
				...
				<Tab eventKey="Pending" title="Pending">
					<Card.Body className="d-flex justify-content-between p-1 my-2">
						<DropdownButton
							...
							<Dropdown.Item as="button" onClick={() => setSelectedItem("Unseen")}>
								Unseen
							</Dropdown.Item>
						</DropdownButton>
						<Button onClick={() => navigate("/admin/viewReports")}>Go back to All Courses</Button>
					</Card.Body>
				...
				<Tab eventKey="Resolved" title="Resolved">
					...
					<Card.Body className="d-flex flex-wrap">
						{currentResolved.map((report, i) => (
							<Col sm={4} className=" p-1 ">
								<ReportCard report={report} seenReport={SeenReport} AddComment={AddComment} />
							</Col>
						))}
					</Card.Body>
					<Pagination
